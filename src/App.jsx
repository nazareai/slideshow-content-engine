                ...
                <div className="grid gap-4 md:grid-cols-2">
                  {slides.map((slide) => <article key={slide.id} className="panel overflow-hidden">
                    ...
                    <div className="p-4"><label className="sr-only" htmlFor={`slide-${slide.id}`}>Slide {slide.id} copy</label><textarea id={`slide-${slide.id}`} className="field min-h-28 resize-y font-display text-xl font-bold leading-snug" value={slide.text} disabled={generatingImages} onChange={(e) => updateSlide(slide.id, 'text', e.target.value)} /><label className="label mt-4" htmlFor={`visual-${slide.id}`}>Visual direction</label><input id={`visual-${slide.id}`} className="field" value={slide.visual} disabled={generatingImages} onChange={(e) => updateSlide(slide.id, 'visual', e.target.value)} /><fieldset className="mt-4 grid min-w-0 gap-3 rounded-xl border border-black/10 bg-black/[0.025] p-3 sm:p-4">
                      ...
                      {['editorial-split', 'takeaway-ledger'].includes(slide.direction?.layout) && <div className="rounded-lg border border-cobalt/25 bg-cobalt/5 p-3">
                        <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
                          <div><span className="label mb-0">Large layout overlay</span><p className="mt-1 text-xs text-black/55">Controls the full-width panel over the generated image, not the text background.</p></div>
                          <button type="button" role="switch" aria-checked={slide.overlay?.panelEnabled ?? true} aria-label={`Slide ${slide.id} large layout overlay`} className={cn('button justify-center', (slide.overlay?.panelEnabled ?? true) && 'bg-cobalt text-white')} onClick={() => updateOverlay(slide.id, { panelEnabled: !(slide.overlay?.panelEnabled ?? true) })}>{(slide.overlay?.panelEnabled ?? true) ? 'Overlay on' : 'Overlay off'}</button>
                        </div>
                        <div className="grid min-w-0 grid-cols-1 gap-3">
                          <RangeControl label="Overlay top edge" ariaLabel={`Slide ${slide.id} layout overlay top edge`} min="20" max="85" step="1" value={slide.overlay?.panelTop ?? 59} valueLabel={`${slide.overlay?.panelTop ?? 59}%`} disabled={!(slide.overlay?.panelEnabled ?? true)} onChange={(e) => updateOverlay(slide.id, { panelTop: Number(e.target.value) })} />
                          <RangeControl label="Overlay opacity" ariaLabel={`Slide ${slide.id} layout overlay opacity`} min="0" max="1" step="0.05" value={slide.overlay?.panelOpacity ?? 1} valueLabel={`${Math.round((slide.overlay?.panelOpacity ?? 1) * 100)}%`} disabled={!(slide.overlay?.panelEnabled ?? true)} onChange={(e) => updateOverlay(slide.id, { panelOpacity: Number(e.target.value) })} />
                          <label className="grid min-w-0 gap-2 text-xs font-semibold"><span>Overlay color</span><input type="color" aria-label={`Slide ${slide.id} layout overlay color`} value={slide.overlay?.panelColor ?? '#f7f5f0'} disabled={!(slide.overlay?.panelEnabled ?? true)} onChange={(e) => updateOverlay(slide.id, { panelColor: e.target.value })} className="h-10 w-full cursor-pointer rounded-md border border-black/15 bg-white p-1 disabled:cursor-not-allowed disabled:opacity-50" /></label>
                        </div>
                      </div>}
                      <button type="button" role="switch" aria-checked={Boolean(slide.overlay?.backgroundEnabled)} aria-label={`Slide ${slide.id} tight text background`} className={cn('button justify-center', slide.overlay?.backgroundEnabled && 'bg-black text-white')} onClick={() => updateOverlay(slide.id, { backgroundEnabled: !slide.overlay?.backgroundEnabled })}>{slide.overlay?.backgroundEnabled ? 'Text background on' : 'Text background off'}</button>
                      <div className="grid min-w-0 grid-cols-1 gap-3">
                        <RangeControl label="Text background opacity" ariaLabel={`Slide ${slide.id} text background opacity`} min="0" max="0.95" step="0.05" value={slide.overlay?.backgroundOpacity ?? 0.7} valueLabel={`${Math.round((slide.overlay?.backgroundOpacity ?? 0.7) * 100)}%`} disabled={!slide.overlay?.backgroundEnabled} onChange={(e) => updateOverlay(slide.id, { backgroundOpacity: Number(e.target.value) })} />
